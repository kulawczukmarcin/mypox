...
"""
Based on topo_proactive.

Changes to be made: during start looking for n shortest paths for each src, dst pair.

topo_proactive description:
...
def _get_path (src, dst):
  ...
  if src == dst:
    ...
    # print src, dst
    # print "orginal path is ", _get_raw_path(src, dst)
    if path is None: return None
    # this is how we we will use all_raw_paths
    # there are some problems with that, but should work somehow
    # print "new ", all_raw_paths[src][dst][0]
    path = [src] + path + [dst]
