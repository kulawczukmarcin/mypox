...
def run(controllers):
    ...
    CLI(net)
    #sleep(30)
    hPoznan, hGdansk = net.getNodeByName('hPoznan', 'hGdansk')
    #print "We ping"
    #hWarszawa.cmd("ping 10.0.0.5 -c 10 > logping.txt")
    sleep(10)
   ...
   # razy 2
    ...
    for i in range(100):
        p = 5000+i
        hGdansk.cmd("iperf -s -u -p  %s  -y C -i 0.5 > wk/normal/server/%s_%s.txt &" % (p, i, flows[i]))

    for i in range(100):
        print "start flow ",i
	p = 5000 + i
        hPoznan.cmd("iperf -c 10.0.0.5 -p %s -u -y C -t %s -b %sm > wk/normal/client/%s &" % (p, times[i]*2, flows[i], i ))
        sleep(sleeps[i]/10)
