...
def run(controllers):
   ...
   # razy 2
    ...
    for i in range(100):
        p = 5000+i
        hGdansk.cmd("iperf -s -u -p  %s  -y C -i 0.5 > wk/app/server/%s_%s.txt &" % (p, i, flows[i]))

    for i in range(100):
        print "start flow ",i
	p = 5000 + i
        hPoznan.cmd("iperf -c 10.0.0.5 -p %s -u -y C -t %s -b %sm > wk/app/client/%s &" % (p, times[i]*2, flows[i], i ))
        sleep(sleeps[i]/10)
